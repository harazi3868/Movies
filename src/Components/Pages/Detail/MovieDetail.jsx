...
function MovieDetail() {
    ...
    useEffect(() => {
      axios
        .get(`https://api.themoviedb.org/3/movie/${id}?api_key=${api}&language=en-US&append_to_response=credits,videos`)
        .then((res) => {
          setMovie(res.data);
        })
        .catch((err) => {
      ...
      <main className="flex flex-col w-full">
        ...
        <div className="absolute flex flex-col md:flex-row gap-5 p-5 mt-[10rem]">
          ...
          <div className="flex flex-col justify-center items-start w-full h-full gap-4 mt-2">
            <h1 className="text-white text-2xl font-bold mt-4">{movie.title}</h1>
            <div className="flex flex-wrap items-center w-full h-full gap-4 mt-4">
              {/* rating and genres */}
              <span className="text-[rgb(248,244,244)] text-2xl font-bold">{movie.vote_average}<i className="fas fa-star" /></span>
              <span className="text-white bg-red-600 p-2 rounded-lg text-xl font-bold">
                {movie.genres.slice(2).map((genre) => genre.name).join(", ")}
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center w-full h-full">
              {/* cast */}
              {movie.credits &&
                movie.credits.cast.slice(0, 5).map((item, index) => (
        ...
        <div className="flex flex-wrap flex-col w-full h-full md:mt-52 mb-10 mt-[58rem]">
          ...
          {movie.videos.results.slice(0, 1).map((item, index) => (
            <iframe
